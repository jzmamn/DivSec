...
public class HolidayController {
    ...
    public String addDivision(@ModelAttribute("maHoliday") Holiday holiday, BindingResult result, ModelMap model) {
        if (result.hasErrors()) {
            logger.error("addPerson", result.getAllErrors());
            return "error/error";
        }
        //System.out.println(holiday.getHolId());

        model.addAttribute("holId", holiday.getHolId());
        model.addAttribute("holDate", holiday.getHolDate());
        model.addAttribute("holDesc", holiday.getHolDesc());
        model.addAttribute("holIsActive", holiday.getHolIsActive());

        if (holiday.getHolId() == null) {
            this.holidaySvc.createHolidaySvc(holiday);
        } else {
            this.holidaySvc.updateHoidaySvc(holiday);
        }
